...
# Load Course Data
# -------------------------
courses = pickle.load(open('D:/MULTI-DOMAIN RECOMMENDER SYSTEM/FRONTEND/rl-rec-sys/Courses/courses.pkl', 'rb'))
similarity = pickle.load(open('D:/MULTI-DOMAIN RECOMMENDER SYSTEM/FRONTEND/rl-rec-sys/Courses/similarity.pkl', 'rb'))
...
def recommend_based_on_likes(liked_courses):
    if not liked_courses:
        # Fallback for first-time users: recommend 50 random courses
        recommended_df = courses.sample(50)
    ...
    for course in liked_courses:
        try:
            # Check if the course is in the dataset
            if course not in courses['course_title'].values:
                st.warning(f"Course '{course}' not found in the dataset.")
                continue

            idx = courses[courses['course_title'] == course].index[0]
            sims = list(enumerate(similarity[idx]))
            all_indexes.extend(sims)

        except Exception as e:
            st.error(f"Error recommending courses based on {course}: {e}")
            continue
    ...
    seen = set(liked_courses)
    recommended = []
    for idx, _ in all_indexes:
        title = courses.iloc[idx].course_title
...
# Streamlit App
# -------------------------
st.set_page_config(page_title="Course Recommender", layout="wide", initial_sidebar_state="expanded")
...
st.write("Your liked courses:", user_likes if user_likes else "None yet")

if st.button("🎯 Recommend Courses", key="recommend", help="Click to get personalized course recommendations!", use_container_width=True):
    recommended = recommend_based_on_likes(user_likes[-3:] if user_likes else ["1 Hour JavaScript"])
    if recommended:
    ...
    for i in range(0, len(st.session_state.recommended), 5):
        ...
        for j in range(5):
            ...
            if idx < len(st.session_state.recommended):
                course = st.session_state.recommended[idx]
                with cols[j]:
                    st.markdown(f"""
    <style>
        .course-box-{idx} {{
        ...
        .course-box-{idx}:hover {{
            box-shadow: 0 0 25px #1e90ff;
            transform: scale(1.05);
        }}
    </style>
    <div class="course-box-{idx}">
        <strong style="font-size: 1em; color: white;">{course}</strong>
    </div>
""", unsafe_allow_html=True)
                    if st.button(f"👍 Like {idx+1}", key=f"like{idx}", use_container_width=True):
                        if course not in user_likes:
                            user_likes.append(course)
                            user_data[username]["liked"] = user_likes
                            save_json(USER_DATA_PATH, user_data)
                            if len(user_likes) > 1:
                                prev = user_likes[-2]
                                q_table = update_q_table(q_table, prev, course, reward=1)
                                save_q_table(username, q_table)
                            st.success(f"Liked {course}")
                            st.rerun()
